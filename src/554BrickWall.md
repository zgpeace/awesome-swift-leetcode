...
class Solution {
    func leastBricks(_ wall: [[Int]]) -> Int {
        var height: Int = 0
        var max: Int = 0
        var vertialPassDic: [Int: Int] = [Int: Int]()
        for rowList in wall {
            height += 1
            var passIndex: Int = 0
            for i in 0..<(rowList.count - 1) {
                passIndex += rowList[i]
                let passCount: Int = vertialPassDic[passIndex] == nil ? 0 : vertialPassDic[passIndex]!
                vertialPassDic[passIndex] = passCount + 1
            }
        }
        for (_, passNo) in vertialPassDic {
            if passNo > max {
                max = passNo
            }
        }
